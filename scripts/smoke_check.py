...
from src.forward_kinematics import Arm4DOFDH
...
from src.joint_space_dynamics import SimpleDynamics4DOF
...
def main() -> None:
    arm = Arm4DOFDH()
    q = np.deg2rad([30.0, 20.0, -15.0, 10.0])

    state = arm.frame_state(q)
    _ensure(state.joint_4_transform.shape == (4, 4), "FK joint-4 transform has wrong shape.")
    _ensure(state.tool_transform.shape == (4, 4), "FK tool transform has wrong shape.")
    _ensure(state.joint_points.shape == (5, 3), "FK joint points have wrong shape.")
    ...
    _ensure(
        ...
        "Tool point should stay fixed when only q4 changes.",
    )

    J = geometric_jacobian(arm, q)
    J_v = position_jacobian(arm, q)
    analysis = analyze_position_jacobian(arm, q)
    _ensure(J.shape == (6, 4), "Geometric Jacobian has wrong shape.")
    _ensure(J_v.shape == (3, 4), "Position Jacobian has wrong shape.")
    ...
    _ensure(np.allclose(J_v[:, 3], 0.0, atol=1e-9, rtol=1e-9), "Default geometry should make the q4 position column zero.")
    ...
    _ensure(q_dot_cmd.shape == (4,), "Inverse kinematics returned the wrong shape.")
    ...
    _ensure(tau.shape == (4,), "Controller returned the wrong torque shape.")
    ...
    q_next, q_dot_next = dyn.step(q, np.zeros(4), tau, dt=0.01)
    _ensure(q_next.shape == (4,), "Dynamics returned the wrong q shape.")
    _ensure(q_dot_next.shape == (4,), "Dynamics returned the wrong q_dot shape.")
    ...
    ts, qs, qdots = simulate_joint_step_response(
        ...
        q_ref=np.deg2rad([10.0, -5.0, 15.0, 0.0]),
        dt=0.01,
        t_final=0.05,
    )
    _ensure(ts.ndim == 1 and qs.shape[1] == 4 and qdots.shape[1] == 4, "Step-response simulation returned inconsistent shapes.")
